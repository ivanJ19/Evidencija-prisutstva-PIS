...
fetch("http://localhost:8080/courses/all", {
    ...
    .then((result) => {
        ...
        for (var item of result["courses"]) {
            list.innerHTML += '<tr><td scope="row">' + item["id"] + '</td><td>' + item["coursename"] + '</td><td>' + item["description"] + '</td><td><button class="btn btn-success btn-sm btn-action" onclick="fnUpdate(' + item["id"] + ')">Ažuriraj</button>' + ((item["id"] > 1) ? '<button class="btn btn-danger btn-sm" onclick="fnDelete(' + item["id"] + ')">Obriši</button>' : "") + '</td></tr>';
        }
    });
...
function fnUpdate(id) {
    ...
    // Redirect page
    window.location.href = "/courses/update.html";
}
...
live("click", "btnYes", function (event) {
    ...
    fetch('http://localhost:8080/courses/delete/' + dataID, {
        method: 'DELETE',
        headers: {
            'Content-Type': 'application/json',
            'Accept': 'application/json'
